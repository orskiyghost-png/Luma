...
export function AdminReports({
  ...
  if (reports.length === 0) {
    return <p className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-400">Жалоб нет.</p>;
  }
      ...
      {reports.map((r) => (
        <div key={r.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" disabled={pending} onClick={() => run(() => setReportStatus(r.id, "reviewing"))} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:border-slate-300 disabled:opacity-50">В работу</button>
            ...
            <button type="button" disabled={pending} onClick={() => run(() => setReportStatus(r.id, "rejected"))} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:border-slate-300 disabled:opacity-50">Отклонить</button>
            ...
            {r.target_author && (
              <>
                ...
                <button type="button" disabled={pending} onClick={() => run(() => setUserBan(r.target_author!, false))} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:border-slate-300 disabled:opacity-50">Разбанить</button>
              </>
            )}

            {canManageRoles && r.target_author && (
              <button type="button" disabled={pending} onClick={() => run(() => setUserRole(r.target_author!, "moderator"))} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-600 hover:border-slate-300 disabled:opacity-50">Сделать модератором</button>
            )}
          </div>
        </div>
      ))}
