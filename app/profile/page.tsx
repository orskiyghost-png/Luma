...
export default async function ProfilePage() {
  ...
  const { data: profile } = await supabase.from("profiles").select("display_name, bio, city, city_visible, avatar_url, avatar_full_url, role, age_verified_adult").eq("user_id", user.id).maybeSingle();
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="grid gap-6 lg:grid-cols-[0.7fr_1.3fr]">
        <aside className="rounded-3xl bg-ink p-6 text-white shadow-glow"><div className="grid h-20 w-20 place-items-center rounded-2xl bg-tide text-3xl font-black text-ink">{(displayName).slice(0, 1).toUpperCase()}</div><p className="mt-7 text-xs font-black uppercase tracking-[0.16em] text-white/50">Аккаунт</p><p className="mt-2 break-all text-sm text-white/80">{user.email}</p><div className="mt-8 rounded-2xl bg-white/10 p-4 text-sm leading-6 text-white/70">{profile?.age_verified_adult ? "Возраст 18+ подтверждён. Показ живой позиции — по вашему выбору, управляется ниже." : "Показ живой позиции по умолчанию выключен и доступен только после подтверждения 18+."}</div></aside>
      ...
      <p className="mt-8 text-center text-xs text-slate-400">
        <Link href="/legal/privacy" className="underline hover:text-ink">Политика конфиденциальности</Link>
        {" · "}
        <Link href="/legal/terms" className="underline hover:text-ink">Пользовательское соглашение</Link>
      </p>
      </div>
    </main>
  );
